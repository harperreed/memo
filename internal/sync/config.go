...
func ConfigPath() string {
	if xdgConfig := os.Getenv("XDG_CONFIG_HOME"); xdgConfig != "" {
		return filepath.Join(xdgConfig, "memo", "sync.json")
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return filepath.Join(os.TempDir(), ".memo", "sync.json")
	}
	return filepath.Join(home, ".config", "memo", "sync.json")
}
...
func applyEnvOverrides(cfg *Config) {
	if server := os.Getenv("MEMO_SYNC_SERVER"); server != "" {
		cfg.Server = server
	}
	if token := os.Getenv("MEMO_SYNC_TOKEN"); token != "" {
		cfg.Token = token
	}
	if userID := os.Getenv("MEMO_SYNC_USER_ID"); userID != "" {
		cfg.UserID = userID
	}
	if vaultDB := os.Getenv("MEMO_SYNC_VAULT_DB"); vaultDB != "" {
